...
export const addPlatform = async (req, res) => {
  try {
    ...
    res.status(200).json({
      success: true,
      message: "Added To Coding Platforms",
    });
  } catch (error) {
    return res.status(400).json({
      success: false,
...
export const deletePlatform = async (req, res) => {
  try {
    ...
    const video = user.platform.find((video) => video._id == id);

    await cloudinary.v2.uploader.destroy(video.image.public_id);

    user.platform = user.platform.filter((video) => video._id != id);

    await user.save();

    res.status(200).json({
      success: true,
      message: "Deleted from Coding Platforms",
    });
  } catch (error) {
    return res.status(400).json({
      success: false,
